...
```{r librerías, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r librerías_1 , message=FALSE, warning=FALSE, include= TRUE, echo=TRUE}
...
archivo <- "Fuentes/0.resumen_macro_eco-finan_enge_1993-2024_Yacovone.v1.xlsx"

# Leer las hojas del archivo
sheets <- c("ENGE_1993a2004", "ENGE_2004a2011", "ENGE_2012a2016", "ENGE_2017a2024")
...
Por un lado, se unificaron los periodos i) y ii) dando lugar a un dataframe que cubre los años que van desde 1993 hasta 2011. Por otro lado, se unificaron los periodos iii) y iv), y con ello se logró abarcar desde 2012 hasta 2024.
<div/> 
...
ENGE_2012a2024 <- merge(x = ENGE_2012a2016,
                        y = ENGE_2017a2024,
                        all = T)

#Quito las que ya no uso
rm(ENGE_1993a2004,ENGE_2004a2011,ENGE_2012a2016,ENGE_2017a2024)
...
sectores <- openxlsx::read.xlsx(
  xlsxFile = "Fuentes/0.resumen_macro_eco-finan_enge_1993-2024_Yacovone.v1.xlsx", 
  sheet = 7)
...
```
<div class=text-justify> 
Los **...** significan desagregaciones inexistentes en 1993-2011 que luego pasaron a ser consideradas en 2012-2024  dentro de los sectores antecedidos.
...
Para tener en claro lo anterior, se tiene en cuenta el **Cuadro 1**. Entonces, en la base conformada, la desagregación sectorial para cada periodo se la reconoce con id = 1, en el caso de  1993-2011, y con id = 2, en el caso de 2012-2024. 
<div/> 
...
base.ENGE_1993a2024.bind <- bind_rows(list(ENGE_1993a2011,ENGE_2012a2024), .id = "id")
```

```{r duplicates}
#ME fijo si hay observaciones repetidas
base.ENGE_1993a2024.bind %>%
...
write.xlsx(base.ENGE_1993a2024.bind,"Output/base.ENGE_1993a2024_wider.id.xlsx")

#Pivoteo
base.ENGE_1993a2024.bind <- base.ENGE_1993a2024.bind %>% 
...
write.xlsx(base.ENGE_1993a2024.bind,"Output/base.ENGE_1993a2024_longer.id.xlsx")

```
<div class=text-justify>
El resultado de esta instancia inicial de trabajo es una base completa de la ENGE que abarca desde 1993 hasta 2022, tanto en su forma horizontal con los sectores en columnas separadas (base.ENGE_1993a2024_wider.id.xlsx) como en su forma vertical con todos los sectores en una sola columna (base.ENGE_1993a2024_longer.id.xlsx).
...
```{r}
df_enge = base.ENGE_1993a2024.bind 
head(df_enge)
```
```{r}
...
#Reestructuro la base pivoteando los indicadores en columnas como variable separadas
 base.ENGE_1993a2024.wider <- base.ENGE_1993a2024.bind %>% 
...
vis_miss(base.ENGE_1993a2024.wider,sort_miss = T,cluster = T,warn_large_data = T)
...
# df_pivot contiene las columnas: anio, sector, y el resto son los indicadores numéricos

df_indicadores <- df_pivot %>%
  
  # PASO 1: Limpieza de 0s a NA en TODAS las columnas numéricas
  # La función 'where(is.numeric)' selecciona todas las columnas que no son 'anio' o 'sector'.
  mutate(across(where(is.numeric), ~na_if(., 0))) %>% 
  
  # PASO 2: Generación de Indicadores
  # El resto del código de cálculo permanece igual, pero ahora opera sobre un dataframe limpio de 0s engañosos.
  mutate(
    VAB.precios.basicos = VAB * ((1 + impuestos.net.subsidios.en.VAB) / 100),
    amortizaciones = amortizaciones.en.VAB * VAB,
    ...
    consumo_intermedio = VBP - VAB.precios.basicos,
    ganancia_antes_de_impuestos = VAB.precios.basicos - amortizaciones - masa_salarial,
    ganancia_despues_impuestos = VAB.precios.basicos - amortizaciones - masa_salarial,
    activo_corriente = act.corriente.sobre.act.total * activo,
    activo_no_corriente = activo - activo_corriente,
...
```{r tasa_ganancia_total_2012_2023}
...
df_indicadores %>% 
  ...
  ggsave(filename = "./Output/tasa.ganancia.total_2012_2023.png",
       dpi = "retina", 
       device = "png",
       width = 10,
       height = 7 )
...
```{r tasa_ganancia_sectores_2012_2023}
#Gráfico de tasa de ganancia
...
df_indicadores %>% 
  ...
  labs(title = "Tasa de ganancia de las grandes empresas en Argentina",
       subtitle = "Desagregación por sectores (2012-2023)",
       y = "%",
       x = "Año",
       caption = "Fuente: ENGE - INDEC") +
  scale_x_continuous(n.breaks = 10) +
  ...
  ggsave(filename = "./Output/tasa.ganancia.sectores_2012_2023.png",
       dpi = "retina", 
       device = "png",
       width = 10,
       height = 7 )
...
# .xlsx file (View JK)
write.xlsx(df_pivot, file = "Output/df_enge_1993_2024_pivot.xlsx")
...
```{r tasa_ganancia_total_1993-2023}
...
df_indicadores.1 %>% 
  ...
  ggsave(filename = "./Output/tasa.ganancia.total_1993_2023.png",
       dpi = "retina", 
       device = "png",
       width = 10,
       height = 7 )
...
glosario <- openxlsx::read.xlsx(
  xlsxFile = "Fuentes/0.resumen_macro_eco-finan_enge_1993-2023_Yacovone.v2.2.1.xlsx", 
